...
char *date_response() {
    ...
    snprintf(output, 100, "%s %s, %d %s %d %d:%d:%d %s\r\n", DATE, DAYS_OF_WEEK[tm->tm_wday], tm->tm_mday,
             MONTH[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour, tm->tm_min, tm->tm_sec, "GMT");
...
char *compile_response(struct Header *header, char *status, int length, char *full_path) {
    ...
    int total = strlen(status_r) + strlen(date) + strlen(MIME) + strlen(content_t) + strlen(content_len) +
                strlen(connection_t) + strlen(keepalive) + strlen(last_modify);
